...
class scale_block(nn.Module):
    ...
    '''
    def __init__(self, rank, win_size, d_model, n_heads, d_ff, depth, dropout, \
        ...
        for i in range(depth):
            self.encode_layers.append(TwoStageAttentionLayer(rank, seg_num, factor, d_model, n_heads, \
                                                        d_ff, dropout))
...
class Encoder(nn.Module):
    ...
    '''
    def __init__(self, rank, e_blocks, win_size, d_model, n_heads, d_ff, block_depth, dropout,
                in_seg_num = 10, factor=10):
        super(Encoder, self).__init__()
        self.encode_blocks = nn.ModuleList()

        self.encode_blocks.append(scale_block(rank, 1, d_model, n_heads, d_ff, block_depth, dropout,\
                                            in_seg_num, factor))
        for i in range(1, e_blocks):
            self.encode_blocks.append(scale_block(rank, win_size, d_model, n_heads, d_ff, block_depth, dropout,\
